      ...
      {sent ? (
        ...
        <form onSubmit={handleSend} className="mt-5 space-y-4">
          <label className="block">
            ...
            <textarea value={message} onChange={(event) => setMessage(event.target.value)} rows={5} maxLength={4000} placeholder="Describe what happened, what you expected, or what would make DxFlow more useful." className="mt-1.5 w-full resize-y rounded-[10px] border border-border bg-surface px-3.5 py-3 text-sm leading-6 outline-none placeholder:text-muted/65 focus:border-accent focus:ring-2 focus:ring-accent/15" />
          </label>
