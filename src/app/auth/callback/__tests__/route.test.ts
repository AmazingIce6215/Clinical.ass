...
describe("Supabase auth callback", () => {
  ...
  it("exchanges a PKCE code and redirects to a safe internal path", async () => {
    ...
    const response = await GET(new Request(
      "https://wardly.test/auth/callback?code=valid-code&next=%2Fsettings%3Fpassword-reset%3D1",
    ));

    expect(exchangeCodeForSession).toHaveBeenCalledWith("valid-code");
    expect(response.status).toBe(307);
    expect(response.headers.get("location")).toBe("https://wardly.test/settings?password-reset=1");
  });
  ...
  it("does not allow an external redirect target", async () => {
    ...
    const response = await GET(new Request(
      "https://wardly.test/auth/callback?code=valid-code&next=%2F%2Fevil.example",
    ));

    expect(response.headers.get("location")).toBe("https://wardly.test/dashboard");
  });
  ...
  it("returns an actionable sign-in error when exchange fails", async () => {
    ...
    const response = await GET(new Request("https://wardly.test/auth/callback?code=expired"));
