...
function buildProfileDiagnosis(patientCase: PatientCase): DiagnosisResult {
      ...
      differentials: [
        {
          ...
          supportingFindings: ["Symptom pattern consistent with common benign causes", "No red flags present"],
          findingsAgainst: ["Limited clinical data available", "Requires further workup"],
        },
        {
          ...
          supportingFindings: ["Must be excluded in any undifferentiated presentation"],
          findingsAgainst: ["No specific features to support this diagnosis currently"],
        },
      ],
      redFlags: [
  ...
  return {
    ...
    differentials: profile.differentials.map((item, index) => ({
      ...
      supportingFindings: index === 0 ? ["Matches presenting complaint pattern", "Common in this demographic"] : ["Considered due to overlap in symptoms"],
      findingsAgainst: index === 0 ? ["Requires confirmation with exam and investigations"] : ["Less likely based on current presentation"],
    })),
