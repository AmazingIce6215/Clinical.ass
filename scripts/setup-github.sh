...
if [[ ! -f package.json ]]; then
  ...
  echo "Restore the full Orizon source before running this script."
  exit 1
fi
...
if git diff --cached --quiet; then
  ...
  git commit -m "Initial commit: Orizon clinical reasoning app"
...
echo "  2. Run:"
echo "     git remote add origin https://github.com/YOUR_USERNAME/orizon.git"
echo "     git push -u origin main"
echo ""
echo "Or with GitHub CLI (after: brew install gh && gh auth login):"
echo "     gh repo create orizon --private --source=. --remote=origin --push"
